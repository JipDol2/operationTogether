...
public class MemberService {
    ...
    public void validateDuplicateMember(SignUpRequestDto signUpRequestDto){
        List<Member> findMembers = memberJpaRepository.findByMemberID(signUpRequestDto);
    ...
    public LoginResponseDto login(LoginRequestDto loginRequestDto) {
        ...
        //Member findMember = memberRepository.findByMemberOne(loginRequestDto.getUserId(),loginRequestDto.getPassword()).orElseThrow(() -> new IllegalArgumentException("회원아이디 혹은 비밀번호를 잘못 입력하셨습니다."));
        if(findMember.getUserId().equals(loginRequestDto.getUserId()) && findMember.getPassword().equals(loginRequestDto.getPassword())){
            ...
            return new LoginResponseDto(jwtToken, findMember.getId());
        }
